    ...
    const LoadingState = () => (
        <div className="flex items-center justify-center h-screen bg-slate-100">
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-200 max-w-md w-full text-center">
                ...
                <p className="text-sm text-slate-500">Profil, Wissensbasis und Oberfläche werden geladen.</p>
            </div>
        </div>
    );
    ...
    const ConfigErrorState = ({ error }) => (
        <div className="flex items-center justify-center h-screen bg-slate-100">
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-200 max-w-md w-full text-center">
                ...
                <h2 className="font-bold text-[#0a192f] text-lg mb-2">Konfiguration nicht verfügbar</h2>
    ...
    const NotConfiguredState = () => (
        <div className="flex items-center justify-center h-screen bg-slate-100">
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-200 max-w-md w-full text-center">
                ...
                <p className="text-sm text-slate-500 mb-4">Passwort, API-Schlüssel, Projektprofil und Wissensbasis müssen im Admin-Bereich eingerichtet werden.</p>
                <a href="admin.php" className="inline-block px-6 py-3 bg-[#e50046] text-white rounded-2xl font-bold hover:opacity-90 transition-all text-sm">Admin-Wizard öffnen</a>
            </div>
        </div>
    );
            ...
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-[9px] text-slate-400 mt-0.5 uppercase tracking-widest">
                        {config?.subtitle || config?.scope_summary || "Dokumentgestützte Beratung mit RAG"}
                    </p>
                </div>
            </div>
    ...
    const AppFooter = ({ onPrivacyClick, onDsbClick }) => (
        <footer className="bg-[#0a192f] text-slate-400 py-3 px-4 border-t border-white/5 flex flex-wrap justify-center gap-4 text-[9px] font-bold uppercase tracking-widest shrink-0">
            <span className="flex items-center gap-2">
                ...
                <span>RAG-gestützter Beratungsassistent</span>
            </span>
            <button
